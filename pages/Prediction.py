...
technical_skill_options = list(
    x_train.loc[:, "Python(ProgrammingLanguage)":"DataVisualization"].columns
)
non_tech_skill_options = ["ProblemSolving", "Communication"]
technical_skill_options = [
    x for x in technical_skill_options if x not in non_tech_skill_options
]
...
technical_skills = st.multiselect(
    label="Pick technical skills",
    options=technical_skill_options,
    default=["AnalyticalSkills"],
)
Non_tech_skills = st.multiselect(
    label="Pick non-tech skills",
    options=non_tech_skill_options,
    default=["ProblemSolving"],
)
...
for col in x.columns:
    if (
        col in job
        or col in technical_skills
        or col in working_type
        or col in location
        or col in Non_tech_skills
    ):
